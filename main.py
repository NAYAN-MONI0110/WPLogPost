...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("command",
                   choices=["enumerate", "bruteforce", "vuln", "auto"],
                   help="Command to run")
    ...
    p.add_argument("--format",
                   choices=list(SUPPORTED_FORMATS), default="txt",
    ...
    p.add_argument("--batch",
                   type=int, default=1,
                   help="XMLRPC batch size (default: 1)")
...
def _merge(base: dict, args: argparse.Namespace) -> dict:
    ...
    for k, v in {
        ...
        "output":  args.output,
        "format":  args.format,
        "safe":    args.safe or base.get("safe", False),
    }.items():
        if v is not None:
            cfg[k] = v
    return cfg
...
def _load_users(path: str | None) -> list[str]:
    ...
    p = Path(path)
    if not p.is_file():
        logging.warning("Usernames file not found: %s", path)
        return []
...
def _cmd_enumerate(args, config) -> list:
    ...
    if not found:
        print(c("  [-]  No users found.", C.DIM))
        return []
    print(c(f"\n  [+]  {len(found)} user(s) discovered:\n", C.GREEN, C.BOLD))
    for u in found:
        sources = ", ".join(sorted(u.sources)) if hasattr(u, "sources") else "?"
        slug    = u.slug if hasattr(u, "slug") else str(u)
        print(f"       {c(slug, C.CYAN, C.BOLD)}  {c(f'[{sources}]', C.DIM)}")
    out = Path("users.txt")
    out.write_text("\n".join(_slugs(found)) + "\n", encoding="utf-8")
    print(c(f"\n  [+]  Saved -> {out}", C.DIM))
    return found
...
def _cmd_bruteforce(args, config, usernames: list[str]) -> list[Credential]:
    if not args.wordlist:
        print(c("  [-]  --wordlist required.", C.RED), file=sys.stderr)
        sys.exit(1)
    ...
    if not usernames:
        ...
        if not usernames:
            print(c("  [-]  No users found.", C.RED))
            return []

    creds: list[Credential] = []
    attack_map = [
        ("xmlrpc",  XMLRPCAttack),
        ("wplogin", WPLoginAttack),
        ("restapi", RestAPIAttack),
    ]
    for vec, Cls in attack_map:
        if args.mode not in (vec, "all"):
            continue
        if creds and args.mode == "all":
            break
...
def _detect_vectors(url: str, config: dict) -> dict[str, bool]:
    ...
    class _Prober(WordPressTesterBase):
        def run(self):
            return []

    vectors: dict[str, bool] = {}
    with _Prober(url, config) as p:
        # XML-RPC
        r = p._request(
            "POST", f"{url}/xmlrpc.php",
...
def _cmd_auto(args, config, usernames: list[str]) -> list[Credential]:
    ...
    for vec, Cls in priority:
        if not vectors.get(vec):
            continue
    ...
    print(c("  [-]  All vectors exhausted.", C.RED))
    return []
...
def main(argv=None) -> int:
    ...
    # Banner always shows on every run
    print_banner(VERSION)

    # Validate and normalise target URL
    try:
        args.url = normalise_url(args.url)
        if not is_valid_http_url(args.url):
            raise ValueError(args.url)
    except ValueError as e:
        print(c(f"  [-]  Invalid URL: {e}", C.RED), file=sys.stderr)
        return 1
    ...
    try:
        if args.command == "enumerate":
            ...
            if config.get("safe"):
                print(c("  [!]  Safe mode — brute-force disabled.", C.YELLOW))
                return 0
        ...
        print(c(f"\n  [-]  Fatal error: {exc}", C.RED), file=sys.stderr)
        return 1
